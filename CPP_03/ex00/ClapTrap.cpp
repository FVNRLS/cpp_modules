...
ClapTrap::ClapTrap(std::string name) : _name(name), _hit_points(10), _energy_points(10), _damage(0) {
	std::cout << "ClapTrap constructor with name " << _name << " called" << std::endl << std::endl;
}
...
ClapTrap::~ClapTrap() {
	if (_name.length() == 0)
		std::cout << std::endl << "default ClapTrap was destroyed" << std::endl;
	else
		std::cout << std::endl << "ClapTrap " << _name << " was destroyed" << std::endl;
}
